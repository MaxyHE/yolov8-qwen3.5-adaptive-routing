...
| **Qwen3.5-0.8B LoRA** | **90.37%** | **+18.18 pp** |

**关键发现：**
- 图像级评估下 LoRA 微调提升仅 +0.15 pp（90.40% → 90.55%），几乎不可见；框级别精确评估下提升达 +11.76 pp（78.61% → 90.37%），说明图像级评估严重掩盖了 LoRA 的真实效果
...
| 配置 | rank | target_modules | 框级别准确率 |
| :--- | :---: | :--- | :---: |
| r4_qv | 4 | q_proj + v_proj | **92.51%** |
| r8_qv | 8 | q_proj + v_proj | 90.37% |
| r16_qv | 16 | q_proj + v_proj | 87.70% |
| r8_qvk | 8 | q_proj + v_proj + k_proj | 92.51% |

**关键发现：**
- rank 越大在 5389 样本规模下反而性能下降，说明小数据集上过拟合风险随 rank 增大而上升，小 rank 的正则效果更重要
- 加入 k_proj（r8_qvk）与 r4_qv 准确率相同，但两者 rank 不同，k_proj 的独立贡献有待进一步控制变量验证；结合原始 LoRA 论文的消融结论，仅加 q/v 是合理的默认选择
